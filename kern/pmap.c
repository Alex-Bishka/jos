...
{
	perm |= PTE_P;
	for (void* current = (void*) va; current < ROUNDUP(va + len, PGSIZE); current = ROUNDDOWN(current + PGSIZE, PGSIZE)) {
		pte_t* pg = pgdir_walk(env->env_pgdir, current, 0);
		if (!pg || ((*pg & perm) != perm) || ((uint32_t) current >= ULIM)) {
